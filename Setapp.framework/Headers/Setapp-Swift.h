...
@interface STPConfiguraton : NSObject
/// The default configuration for the current app.
SWIFT_CLASS_PROPERTY(@property (nonatomic, class, readonly, strong) STPConfiguraton * _Nonnull defaultConfiguration;)
+ (STPConfiguraton * _Nonnull)defaultConfiguration SWIFT_WARN_UNUSED_RESULT;
...
///
/// \param publicKeyFilename A name of a public key file. If <code>nil</code>, the “setappPublicKey.pem<code>"</code>
/// string is used.
/// Default value: <code>nil</code>.
///
...
/// An object that handles Setapp subscriction updates.
...
/// A shared Setapp manager object for an applictaion.
...
@property (nonatomic, strong) STPConfiguraton * _Nonnull configuration;
/// Current log level.
/// Default value: <code>SetappLogLevel.info</code>.
@property (nonatomic) enum STPLogLevel logLevel;
...
///
/// returns:
/// <code>true</code> if the Setapp iOS Framework successfully handled the request to open a URL,
/// or <code>false</code> if the attempt to open the URL resource failed.
- (BOOL)openURL:(NSURL * _Nonnull)url options:(NSDictionary<UIApplicationOpenURLOptionsKey, id> * _Nonnull)options SWIFT_WARN_UNUSED_RESULT;
/// Returns <code>true</code> if at least one of the URL contexts could be openned by the Setapp iOS Framework,
...
@protocol STPManagerDelegate
/// Tells the delegate that the Setapp subcription has been updated.
...
/// A Boolean value that is <code>true</code> if a susbscription is active, otherwise <code>false</code>.
