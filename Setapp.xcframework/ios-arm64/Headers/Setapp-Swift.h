...
SWIFT_CLASS("_TtC6Setapp27AlertPresentationController")
...
@class NSBundle;
@class NSCoder;
SWIFT_CLASS("_TtC6Setapp28AlertPresenterViewController")
...
SWIFT_CLASS("_TtC6Setapp26AlertTransitioningDelegate")
...
SWIFT_CLASS("_TtC6Setapp19AlertViewController")
...
SWIFT_CLASS("_TtC6Setapp24DeclarativeViewContainer")
@interface DeclarativeViewContainer : UIView
- (nullable instancetype)initWithCoder:(NSCoder * _Nonnull)_ OBJC_DESIGNATED_INITIALIZER SWIFT_UNAVAILABLE;
...
/// A facade for all inner managers. The only object the <code>SetappManager</code> communicates with.
/// Responsible for between-managers communication.
SWIFT_CLASS("_TtC6Setapp14ManagersFacade")
@interface ManagersFacade : NSObject
...
@class NSNumber;
@interface ManagersFacade (SWIFT_EXTENSION(Setapp))
- (void)resetToNewSetappConfiguration:(STPConfiguration * _Nullable)newSetappConfiguration includeKeychain:(NSNumber * _Nonnull)includeKeychain;
@end
...
@property (nonatomic, readonly, copy) NSURL * _Nullable fallbackURL;
@property (nonatomic, readonly, copy) NSString * _Nullable activationTypeString;
@property (nonatomic, readonly, copy) NSString * _Nullable marketplaceBundleID;
@property (nonatomic, readonly) BOOL isDismissable;
@property (nonatomic, readonly, copy) void (^ _Nullable onDismiss)(void);
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
+ (nonnull instancetype)new SWIFT_UNAVAILABLE_MSG("-init is unavailable");
@end

typedef SWIFT_ENUM(NSInteger, STPStatusMessageType, open) {
/// Presenter will be notified with this message when activation prompt should be presented.
  STPStatusMessageTypeActivationPrompt = 0,
/// Presenter will be notified with this message when activation process started.
  STPStatusMessageTypeActivationInProgress = 1,
/// Presenter will be notified with this message when activation process succesfully completed.
  STPStatusMessageTypeActivationSuccess = 2,
/// Presenter will be notified with this message when an error occured;
/// if you receive this message type, parent <code>STPStatusMessage</code>’s error property guaranteed to be non-nil.
/// The <code>SetappError.code</code> contains more detailed information.
  STPStatusMessageTypeError = 3,
/// Presenter will be notified with this message when an AI error occurred.
/// The <code>aiError</code> property contains specific AI error information.
  STPStatusMessageTypeAiError = 4,
};

SWIFT_CLASS("_TtC6Setapp12ScrollerView")
...
@class UIActivity;
SWIFT_CLASS("_TtC6Setapp28SetappActivityViewController")
@interface SetappActivityViewController : UIActivityViewController
/// Property is overriden to force <code>.formSheet</code> presentation style on iPadOS.
/// Otherwise it would be presented as a popover which has to have a <code>sourceView</code>.
...
///
+ (BOOL)isSetappBackgroundSessionIdentifier:(NSString * _Nonnull)identifier SWIFT_WARN_UNUSED_RESULT SWIFT_DEPRECATED_MSG("This method is not needed anymore.");
/// Checks if a provided URL session <code>identifier</code> was created by the Setapp framework or not.
/// Returns <code>true</code> if a provided identifier represents background Setapp URL session,
/// otherwise - <code>false</code>.
/// \param identifier The identifier of the URL session to check.
...
@class UIWindowScene;
SWIFT_CLASS("_TtC6Setapp19SetappOverlayWindow")
@interface SetappOverlayWindow : UIWindow
- (void)makeKeyWindow;
...
@class NSURLResponse;
/// Passes delegated methods from the background URL session to another object.
/// We can’t create URLSession and set a delegate later and we have a Chicken-and-egg problem, when we
/// have an object that depends on a background URL session. This class was designed to be a solution
/// for that problem.
SWIFT_CLASS("_TtC6Setapp23URLSessionDelegateProxy")
@interface URLSessionDelegateProxy : NSObject <NSURLSessionDataDelegate>
...
/// An endpoint manager for web services.
/// Fetches an endpoints manifest and updates the endpoints in the corresponding contained web services.
SWIFT_CLASS("_TtC6Setapp18WebServicesManager")
